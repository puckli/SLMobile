...
@Repository("ssoCommonsDAO")
public class SSOCommonsDAOImpl<T> implements SSOCommonsDAO<T>
{
	private static final Logger log = Logger.getLogger(SSOCommonsDAOImpl.class);

	private SessionFactory ssoSessionFactory;

	public SessionFactory getSsoSessionFactory()
	{
		return ssoSessionFactory;
	}
	@Autowired
	public void setSsoSessionFactory(SessionFactory ssoSessionFactory)
	{
		this.ssoSessionFactory = ssoSessionFactory;
	}

	public Session getSession()
	{
		// 事务必须是开启的(Required)，否则获取不到
		Session session = ssoSessionFactory.getCurrentSession();
		return session;
	}
	...
	public List findByHQL(String hql)
	{
		...
		{
			log.error("查询错误！" + SSOCommonsDAOImpl.class.getName() + ".findByHQL " + new Date());
			e.printStackTrace();
		}
		return null;
	}
	...
	public List<T> listByHQL(String hql,int page, int max, Object... args)
	{
		...
		{
			log.error("查询错误！" + SSOCommonsDAOImpl.class.getName() + ".listByHQL " + new Date());
			e.printStackTrace();
		}
		return null;
	}
	...
	public int executeHQL(String hql, Object... args)
	{
		...
		{
			log.error("查询错误！" + SSOCommonsDAOImpl.class.getName() + ".executeHQL " + new Date());
			e.printStackTrace();
		}
		return -1;
	}
