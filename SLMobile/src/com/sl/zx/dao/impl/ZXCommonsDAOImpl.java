...
@Repository("zxCommonsDAO")
public class ZXCommonsDAOImpl<T> implements ZXCommonsDAO<T>
{
	private static final Logger log = Logger.getLogger(ZXCommonsDAOImpl.class);

	private SessionFactory zxSessionFactory;

	public SessionFactory getZxSessionFactory()
	{
		return zxSessionFactory;
	}

	@Autowired
	public void setZxSessionFactory(SessionFactory zxSessionFactory)
	{
		this.zxSessionFactory = zxSessionFactory;
	}

	public Session getSession()
	{
		// 事务必须是开启的(Required)，否则获取不到
		Session session = zxSessionFactory.getCurrentSession();
		return session;
	}
	...
	public List findByHQL(String hql)
	{
		...
		{
			log.error("查询错误！" + ZXCommonsDAOImpl.class.getName() + ".findByHQL " + new Date());
			e.printStackTrace();
		}
		return null;
	}
	...
	public List<T> listByHQL(String hql, int page, int max, Object... args)
	{
		...
		{
			log.error("查询错误！" + ZXCommonsDAOImpl.class.getName() + ".listByHQL " + new Date());
			e.printStackTrace();
		}
		return null;
	}
	...
	public int executeHQL(String hql, Object... args)
	{
		...
		{
			log.error("查询错误！" + ZXCommonsDAOImpl.class.getName() + ".executeHQL " + new Date());
			e.printStackTrace();
		}
		return -1;
	}
