...
.blockwrap{display:inline-block;background-color:rgba(234,207,82,0.1);border-radius:5px;box-shadow:0 0 2px 2px #eee;
	margin:2px 5px 10px;padding:2px 5px;width:100%;min-width: 400px;}
...
<body>
	...
	<section>
		...
		<div class="blockwrap">
			...
			<div class="link-wrap">
				<a href="/slmobile/ios/getUnExamineSldList?uid=<%=session.getAttribute("uid")%>">
				 <span class="wait img">&nbsp;</span> <span class="count">${requestScope.unExamine}</span>条未审单据 </a>
				<a href="/slmobile/ios/getAgreeSldList?uid=<%=session.getAttribute("uid")%>">
				<span class="ok img">&nbsp;</span> <span class="count">${requestScope.agree }</span>条同意单据 </a>
				<a href="/slmobile/ios/getDisagreeSldList?uid=<%=session.getAttribute("uid")%>">
				<span class="refuse img">&nbsp;</span><span class="count"> ${requestScope.disagree }</span>条拒绝单据</a>
			</div>
		</div>
		...
		<div class="blockwrap">
			<p>资质资信</p>
			<div class="link-wrap">
				<a href="/slmobile/zx/goFileOverviewPage?uid=<%=session.getAttribute("uid")%>">文件概览</a>
				<a href="/slmobile/zx/goViewFilePage?uid=<%=session.getAttribute("uid")%>">查询文件</a>
			</div>
		</div>
		...
		<div class="blockwrap">
			<p>收支平衡日报</p>
			<div class="link-wrap">
				<a href="/slmobile/ncreport/search.action?uid=<%=session.getAttribute("uid")%>">查看报表</a>
			</div>
		</div>
	</section>
