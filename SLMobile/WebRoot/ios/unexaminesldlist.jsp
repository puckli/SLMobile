...
<title>unexamine sld list</title>
...
<body>
	...
	<header>
		...
		<a class="title">未审批单据</a>
		<a class="button button-rounded" href="/slmobile/ios/getUnExamineSldList?pageNow=<%=pagePre %>&uid=<%=session.getAttribute("uid") %>">上一页</a> 第${requestScope.pageNow }/${requestScope.pageCount}页 
		<a class="button button-rounded" href="/slmobile/ios/getUnExamineSldList?pageNow=<%=pageNext %>&uid=<%=session.getAttribute("uid") %>">下一页</a>
	</header>
	<section>
			<table>
				<thead>
				...
				<tbody>
					<% int row = 1; %>
					<s:iterator value="#request.ueList">
					<tr onclick="viewAndExamine(<s:property value="sid"/>);">
						<td>
							<%=row++ %>
						</td>
						<td>
	...
	<script type="text/javascript">
		...
		function viewAndExamine(sid)
		{
			location.href="/slmobile/ios/viewAndExamine?uid=" + '${requestScope.uid}' + "&sid=" + sid + "&pageNow=" + ${requestScope.pageNow};
		}
	</script>
</body>
</html>
